...
Log_::Log_(const std::string &fileName, Log_::Level l)
    : level(l)
{
    if (level < MinLevel)
        return;
...
Log_::Log_(const Alias &alias, Log_::Level l)
    : level(l)
{
    if (level < MinLevel)
        return;
...
void initLog() {
    ...
    const QString logFile2 = makePath(QCoreApplication::applicationDirPath(), "log.txt");
